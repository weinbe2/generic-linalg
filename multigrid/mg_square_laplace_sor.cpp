...
// C++ file for multigrid on square laplace using SOR for internal iteration.
...
int main(int argc, char** argv)
{
  ...
  if (argc == 5)
  {
    ...
    ss << argv[4]; // number of smoother steps.
    ss >> smoother_steps;
    ss.clear();
  }
  else
  {
    ...
    cout << "Call as: ./mg_square_laplace.cpp [length] [mass] [levels] [n_smooth]\n";
    ...
    cout << "   [n_smooth]:  The number of smoothing steps to use.\n";
    return 0;
  }
  ...
  while (resid/bnorm > 1e-6)
  {
    ...
    for (p.level=0;p.level<nlev;p.level++) // go down.
    {
      ...
      invif = minv_vector_sor(phi[p.level], res[p.level], p.size[p.level]*p.size[p.level], smoother_steps, 1e-10, omega, square_laplacian, &p);
